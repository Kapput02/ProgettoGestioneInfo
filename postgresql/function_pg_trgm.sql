...
BEGIN
    ...
    WHERE 
        ...
        OR (phrase_search = FALSE AND (
            (field_query = 'title' AND similarity(d.title, processed_query) > 0.23)
            OR (field_query = 'summary' AND similarity(d.summary, processed_query) > 0.23)
            OR (field_query = 'content' AND similarity(d.content, processed_query) > 0.23)
            OR (field_query = 'rating' AND similarity(CAST(d.rating AS TEXT), processed_query) > 0.23)
            OR (field_query IS NULL AND (
                similarity(d.title, processed_query) > 0.23 OR
                similarity(d.summary, processed_query) > 0.23 OR
                similarity(d.content, processed_query) > 0.23 OR
                similarity(CAST(d.rating AS TEXT), processed_query) > 0.23
            ))
        ))
    ORDER BY rank DESC
    LIMIT limit_num;
